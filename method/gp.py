...
from method.optimization import minimize_with_restarts, sample_then_minimize
...
class GP(object):
    ...
    def predict_latent(self, x_star: np.ndarray,
        ...
        # k_star_star is the noise of f, sigma_f
        # sigma_f is not a hyperparameter and does not need to be optimized.
        # To speed up the computation, sigma_f is fixed to 1

        # k_star_star = kern.K(x_star, x_star)
        k_star_star = np.ones(shape=x_star.shape[0])
        mu = k_star.dot(self.alpha)
        var = k_star_star - np.diag(k_star.dot(self.Ka_inv.dot(k_star.T)))
        # Need to copy, else np.diag returns read-only array
        # as sigma_f is fixed, the shape of sigma_f is the same as diagonal
        # if not full_cov:
