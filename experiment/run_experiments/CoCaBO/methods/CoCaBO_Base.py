...
# from utils.initDesignDomain import initBO
...
class CoCaBO_Base(BaseBO):
    ...
    def runTrials(self, trials, budget, saving_path):
        ...
        for i in range(trials):
            ...
            # design_obj = initBO(objfn=self.f, bounds=self.bounds, C=self.C, saving_path=self.saving_path)
            self.trial_num = i
            np.random.seed(i)
            random.seed(i)
    ...
    def save_progress_to_disk(self, best_vals, debug_values, mix_values,
                              saving_path, df):
        result_path = saving_path + "result_data/" + self.f.__name__ + "/"
        # Verify that the saving folder exists, if not, then create it
        if not os.path.exists(result_path):
            os.mkdir(result_path)
        import datetime
        time_flag = str(datetime.datetime.now())[-5:]
        # print("current_time:    " + str(current_time)[-5:])
        results_file_name = result_path + self.name + \
                            f'_{self.batch_size}' + \
                            '_best_vals_' + \
                            self.acq_type + \
                            '_ARD_' + str(self.ARD) + '_mix_' + \
                            str(self.mix) + time_flag
        ...
        # save the results of first self.trial_num trials
    ...
    def compute_reward_for_all_cat_variable(self, ht_next_batch_list, batch_size):
        ...
        for b in range(batch_size):
            ...
            for i in range(len(ht_next_list)):
                ...
                print('ht_reward%d = %f'% (i, ht_reward))
    ...
    def update_weights_for_all_cat_var(self, Gt_ht_list, ht_batch_list, Wc_list, gamma_list,
        ...
        for j in range(len(self.C_list)):
            ...
            if batch_size > 1:
                ...
                # when Gt_ht is very small, such as -3000, thus np.exp(estimatedReward) will be None
                if Wc[ht] is None:
                    Wc[ht] = 0
        return Wc_list
